...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        if (req.method === 'GET') {
            ...
            return res.status(200).json({
                activeSystemPrompt: actionConfig.activeSystemPrompt,
                activeInstructionPrompt: actionConfig.activeInstructionPrompt
            });
        }

        if (req.method === 'POST') {
            const { folder, action, promptPath, type } = req.body;

            if (!folder || !action || !promptPath || !type) {
                return res.status(400).json({ error: 'Missing required parameters' });
            }

            if (type !== 'system' && type !== 'instruction') {
            ...
            if (type === 'system') {
                config.actions[action].activeSystemPrompt = promptPath;
            } else {
                config.actions[action].activeInstructionPrompt = promptPath;
            }
